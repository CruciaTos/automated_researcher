...
from app.schemas.research_job import JobCreate, JobResponse
from app.workers.research_worker import process_research_job
...
def get_job_report(job_id: int, db: Session = Depends(get_db)):
    """
    Return the full generated report for a completed job.

    The `report` field is `null` until the job reaches `completed` status.
    """
    job = db.query(ResearchJob).filter(ResearchJob.id == job_id).first()
    if not job:
        raise HTTPException(
