...
class _DesktopDashboardScreenState
  ...
  Widget _greeting() {
    ...
    return Column(
      ...
      children: [
        Text('$greeting 👋',
            style: Dt.pageTitle.copyWith(fontSize: 24)),
        const SizedBox(height: 4),
        const Text('Your research workspace is ready.',
            style: Dt.bodyMd),
      ],
    );
  }
  ...
  Widget _recentJobsSection(
      AsyncValue<List<ResearchJob>> jobsAsync) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SectionHeader(
        ...
        jobsAsync.when(
          ...
          error: (e, _) =>
              Text('Failed to load jobs: $e', style: Dt.bodySm),
          data: (jobs) {
            if (jobs.isEmpty) {
              return EmptyState(
                ...
                subtitle:
                    'Start your first job using the quick-start panel.',
              );
            }
          ...
          child: ElevatedButton.icon(
            onPressed: creation.isLoading
                ...
                : () async {
                    ...
                    final job =
                        ref.read(jobCreationProvider).value;
                    if (job != null && mounted) {
                      _topicCtrl.clear();
            ...
            icon: creation.isLoading
                ...
                : const Icon(
                    Icons.rocket_launch_outlined, size: 16),
            label: Text(
              creation.isLoading
                  ? 'Starting…'
                  : 'Start Research',
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: const [
              ...
              Expanded(
                  flex: 4,
                  child: Text('TOPIC', style: Dt.sectionLabel)),
              Expanded(
                  flex: 2,
                  child:
                      Text('STATUS', style: Dt.sectionLabel)),
              Expanded(
                  flex: 2,
                  child: Text('PROGRESS',
                      style: Dt.sectionLabel)),
              Expanded(
                  flex: 2,
                  child: Text('STARTED',
                      style: Dt.sectionLabel)),
        ...
        ...jobs.asMap().entries
            .map((e) => _JobRow(job: e.value, index: e.key)),
      ],
    );
  }
}
