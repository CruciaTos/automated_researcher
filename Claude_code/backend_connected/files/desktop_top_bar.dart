      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            healthAsync.when(
              data: (ok) => _BackendChip(
                  connected: ok,
                  onRefresh: () => ref.invalidate(backendHealthProvider)),
              loading: () => const _BackendChip(connected: null),
              error: (_, __) => _BackendChip(
                  connected: false,
                  onRefresh: () => ref.invalidate(backendHealthProvider)),
            ),
          ],
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(Dt.inputRadius),
              border: Border.all(
                  color: _hovered ? Dt.primaryMid : Dt.border),
            ),
            child: Row(
...
class _BackendChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (connected == null) {
      bg = Dt.bgMuted; fg = Dt.textMuted;
      icon = Icons.sync_rounded; label = 'Checking';
    } else if (connected!) {
      bg = Dt.successBg; fg = Dt.success;
      icon = Icons.circle; label = 'Connected';
    } else {
      bg = Dt.errorBg; fg = Dt.error;
      icon = Icons.circle; label = 'Offline';
    }
