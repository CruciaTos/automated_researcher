                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Spacer(),
                      jobsAsync.when(
                        data:    (jobs) => _CountBadge('${jobs.length}'),
                    ...
                    data: (jobs) {
                      if (jobs.isEmpty) {
                        return EmptyState(
                          ...
                          title:    'No jobs yet',
                          subtitle: 'Start a research job to see it here.',
                        );
                      }
          ...
          child: selectedId == null
              ? Center(
                  child: EmptyState(
                    ...
                    title:    'Select a job',
                    subtitle: 'Click a job from the list to see its details.',
                  ),
                )
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return jobState.when(
      loading: () => const Center(
          child: CircularProgressIndicator(strokeWidth: 2)),
      error: (e, _) => Center(
          child: Text('Failed to load job: $e', style: Dt.bodyMd)),
...
class _JobDetailContent extends StatelessWidget {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 12),
              Text('${job.progress}%',
                  style: Dt.cardTitle.copyWith(color: Dt.primary)),
            ],
          ),
                ...
                child: DesktopCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      _detailRow('Job ID', '#${job.id}'),
                      _detailRow('Depth', '${job.depthMinutes} minutes'),
                      _detailRow('Progress', '${job.progress}%'),
                      _detailRow('Status', Dt.statusLabel(job.status)),
                    ],
                  ),
                ),
              ),
                ...
                Container(
                  ...
                  decoration: const BoxDecoration(
                    ...
                    borderRadius: BorderRadius.only(
                      bottomLeft:  Radius.circular(Dt.cardRadius),
                      bottomRight: Radius.circular(Dt.cardRadius),
                    ),
                  ),
  ...
  List<Widget> _stages(ResearchJob job) {
    ...
    final currentIdx = stageOrder.indexOf(job.status);
