...
def create_job(
    ...
    Pass `model_override` to use a specific locally-available Ollama model
    for this job instead of the server default.
    """
    job = ResearchJob(
        topic=payload.topic,
        ...
        status="queued",
        progress=0,
        model_override=payload.model_override,  # ← persisted on the job
    )
    db.add(job)
    db.commit()
    db.refresh(job)
    ...
    logger.info(
        "Created job id=%d topic=%r model_override=%r",
        job.id, job.topic, job.model_override,
    )
    return job
...
def list_jobs(
    ...
    return (
        ...
        .order_by(ResearchJob.id.desc())   # newest first for history view
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def chat_with_job(
    ...
    if not chunks:
        raise HTTPException(
            ...
            detail="No chunks found for this job.",
        )
    ...
    # Use the same model that ran the pipeline, if specified
    answer = generate(prompt, model=job.model_override or None)
